      ...
      <tbody>
        {tracks.map((track, index) => (
          ...
          >
            ...
            <td>{track.bpm ? <span className="musician-badge bpm">{track.bpm} BPM</span> : "-"}</td>
            <td>{track.key ? <span className="musician-badge key">{track.key}</span> : "-"}</td>
            <td style={{ textAlign: "right", color: "var(--text-muted)" }}>
              {formatDuration(track.duration)}
            </td>
            {renderExtraActions && (
              <td style={{ position: "relative" }}>{renderExtraActions(track)}</td>
            )}
          </tr>
        ))}
      </tbody>
